...
![Version](https://img.shields.io/badge/version-1.x-blue) ![Platform](https://img.shields.io/badge/platform-Windows%2010%2B-lightgrey) ![Framework](https://img.shields.io/badge/.NET-10-purple) ![License](https://img.shields.io/badge/license-All%20Rights%20Reserved-red)
...
| 语言 / 运行时 | C# 13 / .NET 10 |
| 架构模式 | MVVM + 依赖注入（CommunityToolkit.Mvvm 8.4） |
| 本地存储 | LiteDB（嵌入式 NoSQL） |
| 加密体系 | DPAPI + AES-256-GCM（令牌凭据 / 资源缓存 / 导出文件） |
| 实时通信 | SignalR Client（通知推送） |
| 远程方案 | WebView2 / RDP 客户端内嵌 / openvpn.exe 子进程 |
| 打包分发 | MSIX / Inno Setup 安装包（x64 / ARM64 / x86） |
...
│                服务层（DI 管理）              │
│   登录认证 · 令牌 · 实例 · 方案 · 自动登录      │
│   通知(SignalR) · 评论 · 信息流 · OpenVPN      │
├──────────────────────────────────────────────┤
│              数据层 / 加密体系                │
│   LiteDB 集合 · DPAPI + AES-256-GCM          │
└──────────────────────────────────────────────┘
```
...
本软件为**闭源商业/专有软件，保留所有权利（All Rights Reserved）**。
